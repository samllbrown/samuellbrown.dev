...
(function () {
	...
	function mountLive(root) {
		...
		function drawPlace() {
			...
			var below = 0, scored = 0; L.districts.forEach(function (o) { var q = o.p[day]; if (q == null) return; scored++; if (q < p) below++; });
			var times = p == null ? 0 : p / L.thresholds.base_rate, timesTxt = times >= 10 ? times.toFixed(0) : times.toFixed(1);
			var tiles = p == null ? '<div class="bl-tile"><b class="off">no data</b><span>no weather for this district on this day</span></div>' :
				'<div class="bl-tile"><em>' + DOT + 'will an outbreak be reported in ' + d.oc + ' in the 7 days from ' + fmtDay(shown) + '?</em><b>' + pct(p, p < 0.01 ? 2 : 1) + '</b><span>my model’s chance, ' + timesTxt + ' times the season average of ' + pct(L.thresholds.base_rate, 1) + ', and higher than ' + pct(below / (scored || 1)) + ' of districts on this day</span></div>' +
				'<div class="bl-tile"><em>' + RING + 'has the weather in ' + d.oc + ' been right for blight?</em><b class="' + (a ? '' : 'off') + '">' + (a ? 'yes' : 'no') + '</b><span>the Hutton alert is ' + (a ? 'on: two days running met the criteria within the last fortnight (' + d.hd14[day].toFixed(0) + ' of its 14 days qualified)' : 'off: no two days running have met the criteria in the last fortnight (' + d.hd14[day].toFixed(0) + ' single days did)') + '</span></div>' +
