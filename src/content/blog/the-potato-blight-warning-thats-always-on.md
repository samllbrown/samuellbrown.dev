...
This map comes from my own service rather than from BlightSpy, and the page fetches it fresh each time it loads. Every morning a GitHub Action in the [blight-forecast](https://github.com/samllbrown/blight-forecast) repo pulls this season's reports from the Fight Against Blight API, pulls a fortnight back and eight days ahead of hourly weather for all 525 districts from Open-Meteo, builds the same features as the research panel, scores every district for today and the week ahead with the model fitted on 2006 to 2025, and commits the result as [one JSON file](https://github.com/samllbrown/blight-forecast/blob/master/live/latest.json). The map shows two different things at once, and they answer different questions. The official warning is a statement about the weather, and my model is a bet on whether an outbreak gets reported, so a district can be under alert with a low chance, or the other way round.

<div class="fc-methods bl-two">
  <div><b><svg viewBox="0 0 20 20" width="18" height="18" aria-hidden="true"><circle cx="10" cy="10" r="7" fill="none" stroke="#e9e6dd" stroke-opacity="0.7" stroke-width="1.5"/></svg>Has the weather here been right for blight?</b><span>The Hutton alert, the official warning, answers yes or no. Yes when two days running had a minimum of 10°C and six hours at 90% humidity, and I keep it at yes for the 14 days after, the same rule the rest of the page scores. It looks only at the weather, so it says nothing about whether blight is actually about, and in a wet July it is yes almost everywhere.</span></div>
  <div><b><svg viewBox="0 0 20 20" width="18" height="18" aria-hidden="true"><circle cx="10" cy="10" r="6" fill="#c561f6"/></svg>Will an outbreak be reported here in the next seven days?</b><span>My model answers with a chance, 0 to 100%, for each district. It reads what week it is, how many outbreaks the scouts have reported nearby, how often this district has had one before, and the weather. The chances are small because a confirmed outbreak in any one district in any one week is rare (the season average is under 1%), so the map colours each district by where its chance sits against all the others today, and the postcode panel says how many times the average it is.</span></div>
</div>
...
<style>
  ...
  .bl-tiles { display: grid; grid-template-columns: 1.5fr 1.5fr 1fr 1fr; gap: 0.6rem; margin-top: 0.7rem; }
  ...
  .bl-tile em { font-style: normal; font-family: var(--font-mono); font-size: var(--text-sm); color: var(--gray-400); display: flex; align-items: flex-start; gap: 0.35rem; line-height: 1.4; }
  .bl-tile em svg { flex: none; margin-top: 0.2em; }
